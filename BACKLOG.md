...
- [ ] **P2 — Distinguer le code de production des tests dans l'analyse directe et les règles.** `microservices-kafka-mq` contient un `@KafkaListener` de test en plus du consommateur de production. Les rapports doivent exclure ces signaux par défaut et permettre de les inclure explicitement. **Vérifié le 17/07** : les fichiers `src/test` ne sont pas indexés (0 fichier de test sur 68), le `KafkaListenerBean` de test est absent de l'inventaire — l'exclusion tient au niveau fichier pour ce dépôt.
...
## Boucle du 17 juillet 2026 — `microservices-kafka-mq` (réindexation + correctif)

Réindexation complète du dépôt révélateur (`rm -rf .cccr && cccr init && cccr index --full --semgrep`). Note `cccr` : **4,5/5** (cf. `reports/microservices-kafka-mq.md`). Un seul écart confirmé du périmètre HTTP a été corrigé cette boucle ; les autres sont documentés ci-dessous.

- [x] **P2 — Détecter les endpoints Spring Data REST exposés par défaut (sans `@RepositoryRestResource(path=…)`).** `microservice-order` dépend de `spring-boot-starter-data-rest` ; `UserRepository extends JpaRepository<User,…>` (sans annotation) est auto-exposé sur `/users` au runtime mais restait invisible (aucun littéral de chemin). Corrigé dans `src/ccc_radar/scanner.py::_infer_spring_data_rest_endpoints` : dérivation du chemin par pluralisation de l'entité, **gâtée sur la dépendance data-rest** (`_module_has_spring_data_rest`) pour éviter le faux positif côté `invoicing` (qui n'a pas data-rest). Régressions : `tests/test_rest_endpoints.py` (3 tests : default-path avec data-rest, suppression sans data-rest, `exported=false`). Vérifié après réindexation : `/users` CRUD apparaît côté order, `/invoices` reste absent côté invoicing.
- [ ] **P2 — Couvrir les ressources de recherche Spring Data REST.** Les méthodes `@Query`/finders des repositories exportés génèrent `/<base>/search/<méthode>` (ex. `/order/search/lastUpdate` sur `OrderRepository.lastUpdate()`). Non couvert aujourd'hui. Dépôt révélateur : `microservices-kafka-mq`. Fichier pressenti : `src/ccc_radar/scanner.py::_infer_spring_data_rest_endpoints`.
- [ ] **P2 — Séparer endpoints framework et APIs métier dans la vue de synthèse.** `microservices show`/`http_apis_exposed` mêle `GET /actuator/**` (`spring-actuator`) et `GET /swagger-ui.html` (`swagger-ui`) aux routes métier, bien que la table `endpoints` les distingue déjà par `framework`. Critère : filtrer ou marquer ces endpoints framework dans la sortie de synthèse, sans les supprimer de la base.
- [ ] **P2 — Couvrir les mappings Spring MVC method-level sans dépendre du pack Semgrep.** Par défaut, `cccr index` n'exécute pas Semgrep ; les `@GetMapping`/`@PostMapping`/`@RequestMapping(method=)` (dont `POST/GET /api/order`) ne sont alors pas détectés — seul le pack `rest` les fournit. Le détecteur local ne fusionne pas le préfixe de classe `@RequestMapping` pour ces méthodes (limitation documentée `scanner.py:440-449`). Critère : le `cccr index` par défaut détecte ces mappings, ou le comportement est explicité à l'utilisateur.
- [ ] **Prérequis outillage (hors régression de cette boucle) — 2 tests `parse_semgrep_kafka_endpoint` en échec sur le baseline `d342d7b`.** `tests/test_rest_endpoints.py::test_parse_semgrep_kafka_endpoint_does_not_depend_on_restclient_state` et `…_unwraps_spring_topic_expressions` échouent avant et après le correctif SDR (analyse Kafka depuis du JSON Semgrep, indépendante du changement). À traiter séparément.
...
| microservices-kafka-mq | **réindexé 17/07** : 2 Kafka (1/1), 19 REST, 1 arête Kafka, 0 HTTP, Mongo 0 | producteur/consommateur `order` (types Order/Invoice), SDR `/order` + `/users` ; ancien index v7 obsolète, régénéré |

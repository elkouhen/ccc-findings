...
| Opérations Mongo | `apiLogRepository.save:96` → `apiLog`; `customerRepository.findAll:61`, `findById:76,96,122`, `delete:126`, `save:141` → `customer` | — | — |
| Arêtes | aucune : aucune cible HTTP ni topic Kafka résolu | — | — |

Note `cccr` finale : **5/5**. La couverture service/HTTP/Kafka/Mongo/graph est complète, avec preuve locale sur chaque opération Mongo. Aucun protocole hors périmètre n'est observé.
